...
for NNN=1:length(division_number)
    for N=0:(total_FOV_number-1)
        ...
        file_path=[folder_path sprintf('%08d',division_number(NNN))];
        fin=fopen(file_path);
        ...
        imwrite(Normailzed_image,[cd,'\divide\',sprintf('stiched_image_%.1f micron',division_number(NNN)*3.2),'.png']);
        stiched_image=zeros(frame_width_eff*X_mosaic_number+X_overlapping,frame_height_eff*Y_mosaic_number+Y_overlapping);
        disp(NNN);

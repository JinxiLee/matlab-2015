...
Reduced_Volume=temp_frame_volume(:,:,Start_Searching_Index:End_Searching_Index);
...
%% Calculating local feature

Sample_Size=30;  %size of the matrix, must be odd number
...
Half_Sample_Size=1;  %size of the matrix, must be odd number
N=3;

for p=(Half_Sample_Size+1):(size(max_index_map,1)-Half_Sample_Size)
    for q=(Half_Sample_Size+1):(size(Height_map,2)-Half_Sample_Size)
        Local_Sample=max_index_map((p-Half_Sample_Size):(p+Half_Sample_Size),(q-Half_Sample_Size):(q+Half_Sample_Size));
        Mean=mean(Local_Sample(:));
        Std=std(Local_Sample(:));
        if abs(max_index_map(p,q)-Mean)>N*Std
            max_index_map(p,q)=Previous_Value;
        end
        Previous_Value=max_index_map(p,q);
    end
end
...
for p=1:size(Reduced_Volume,1)
    for q=1:size(Reduced_Volume,2)
        if max_index_map_Reduced(p,q)+First_index+Second_index<size(Reduced_Volume,3)
            First_Zone(p,q,:)=Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+1):(max_index_map_Reduced(p,q)+First_index));
            Second_Zone(p,q,:)=Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+First_index+1):(max_index_map_Reduced(p,q)+First_index+Second_index));
        elseif max_index_map_Reduced(p,q)+First_index<size(Reduced_Volume,3)
            First_Zone(p,q,:)=Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+1):(max_index_map_Reduced(p,q)+First_index));
            Second_Zone(p,q,1:size(Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+First_index+1):end),3))=Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+First_index+1):end);
        else
            First_Zone(p,q,1:size(Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+1):end),3))=Reduced_Volume(p,q,(max_index_map_Reduced(p,q)+1):end);
    
        end
    end
end

Difference_Map=mean(First_Zone,3)-mean(Second_Zone,3);

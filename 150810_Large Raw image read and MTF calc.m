...
%for p=1:length(file_list)
    p=1;
    TheLargeImage=imread('star','png');
    TheLargeImage=double(TheLargeImage(:,:,1));
    TheSmallImage=TheLargeImage/max(max(TheLargeImage));
    ...
    FFT_image=fft2(Normailzed_Image);
    plot(abs(FFT_image(:,800)));
%end

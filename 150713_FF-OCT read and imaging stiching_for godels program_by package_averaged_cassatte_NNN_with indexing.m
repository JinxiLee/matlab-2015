...
division_starting_index=0;
total_division_number=26;
...
temp_frame_volume=zeros(frame_width,frame_height,num_of_frame_per_division);
...
for NNN=1:length(division_number)
    for N=0:(total_FOV_number-1)
        ...
        folder_path=sprintf('D:\\OCT data\\150713\\2015_0713_150713_musle large area_cassatte_4x_ %d_ %d\\',X_number,Y_number);
        cd(folder_path);
        mkdir('divide_indexed');

        file_path=[folder_path sprintf('%08d',division_number(NNN+indexing(X_FOV_number+1,Y_FOV_number+1)))];
        fin=fopen(file_path);
        ...
        if fin ==-1
            ...
            for q=1:num_of_frame_per_division
                temp_frame_volume(:,:,q)=A(:,(frame_height*(q-1)+1):frame_height*q).*correction_image;
            end
            fclose('all');
        end
        ...
        imwrite(Normailzed_image,[cd,'\divide_indexed\',sprintf('stiched_image_%.1f micron',division_number(NNN)*3.2),'.png']);
        stiched_image=zeros(frame_width_eff*X_mosaic_number+X_overlapping,frame_height_eff*Y_mosaic_number+Y_overlapping);
        disp(NNN);

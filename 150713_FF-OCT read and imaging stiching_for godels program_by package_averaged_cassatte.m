...
for NNN=1:length(division_number)
    for N=0:(total_FOV_number-1)
        ...
        folder_path=sprintf('D:\\OCT data\\150713\\2015_0713_150713_musle large area_cassatte_ %d_ %d\\',X_number,Y_number);
        cd(folder_path);
        mkdir('divide');
        ...
        imwrite(Normailzed_image,[cd,'\divide\',sprintf('stiched_image_%d.png',NNN),'.png']);
        stiched_image=zeros(frame_width_eff*X_mosaic_number+X_overlapping,frame_height_eff*Y_mosaic_number+Y_overlapping);
        disp(NNN);

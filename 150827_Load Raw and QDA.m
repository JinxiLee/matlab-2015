...
for p=1:Number_of_Class
    ...
    %Feature: wighted std (normalized)
        ...
        elseif q>1
            Features{p,2}(q)=Features{p,2}(q-1);
        elseif p>1
            Features{p,2}(q)=Features{p-1,2}(q);
        else
            Features{p,2}(q)=(sum(((Temp(:)-Features(p,q,1)).^2).*Weight_mask(:)).^0.5)/max(Features{p,1}(q));
        end   
    %Feature: Kurtosis
...
for p=1:size(Feature_Map,1)
    for q=1:size(Feature_Map,2)
        ...
        %Feature: wighted std
        if Feature_Map(p,q,1)~=0
            Feature_Map(p,q,2)=(sum(((Temp(:)-Feature_Map(p,q,1)).^2).*Weight_mask(:)).^0.5)/Feature_Map(p,q,1);
        elseif q>1
            Feature_Map(p,q,2)=Feature_Map(p,q-1,2);
        elseif p>1
            Feature_Map(p,q,2)=Feature_Map(p-1,q,2);
        else
            Feature_Map(p,q,2)=(sum(((Temp(:)-Feature_Map(p,q,1)).^2).*Weight_mask(:)).^0.5)/max(Feature_Map(:,:,1));
        end       
        %Feature: Kurtosis
...
Color_2=[0;1;0];
Color_3=[0;0;1];
Color_map_1=zeros(size(Color_map,1),size(Color_map,2),3);
Color_map_2=zeros(size(Color_map,1),size(Color_map,2),3);
Color_map_3=zeros(size(Color_map,1),size(Color_map,2),3);

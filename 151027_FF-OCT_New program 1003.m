...
folder_path_without_index='D:\OCT data\151027\151027_Temp blood\';
...
elseif correction_set == 2
    ...
    start_index_offset_from_max=-35;
    G_Ratio=1.5;
    Gaussian_X_width=400*G_Ratio;
...
X_mosaic_starting_index=6;%6;%1;  %start from 0
Y_mosaic_starting_index=8;%8;%1;


X_mosaic_number=6;%6;%3;%12;%12;
Y_mosaic_number=8;%8;%4;%16;%16;
...
%% glass interface searching
    ...
    for p=1:size(glass_interface_index_map_set,1)
        for q=1:size(glass_interface_index_map_set,2)
            glass_interface_index_map_set(p,q)=glass_interface_index_map_set(p,q)+X_incre*q+Y_incre*p;
        end
    end
...
for NNN=1:total_stack_number
    ...
    for N=0:(total_FOV_number-1)
        ...
        if if_glass_interface_searching==1
            ...
            division_end_index=     floor((start_index_offset_from_max+glass_interface_index_map_set(X_FOV_number+1,Y_FOV_number+1)+(NNN-1)*stack_sampling_spacing+frame_average_factor)/num_of_frame_per_division);
        else

...
division_starting_index=55;
total_division_number=20;

frame_starting_index=1;
total_frame_number=15;
...
Gaussian_X_width=350;
Gaussian_Y_width=200;
X_offset=-100;
Y_offset=-50;
...
for N=0:(total_FOV_number-1)
    ...
    X_FOV_number=2-rem(N,X_mosaic_number); %0~2
    Y_FOV_number=floor(N/X_mosaic_number); %0~2

    
    folder_path=sprintf('D:\\OCT data\\150628_P4 Pork\\%d\\bin\\',N+1);
    cd(folder_path);
    mkdir('divide');


    for p=1:length(division_number)
        file_path=[folder_path sprintf('%08d.bin',division_number(p))];
        fin=fopen(file_path);
